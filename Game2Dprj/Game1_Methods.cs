...
namespace Game2Dprj
{
    ...
    {
        //Update the view in the background in relation to the mouse movements and background limits
        public static void CameraMovement(ref Rectangle viewSource, Point mouseDiff, Point screenDim, Point background) 
        {
            ...
            if (viewSource.X < background.X - screenDim.X && mouseDiff.X > 0)
            {
                viewSource.X += mouseDiff.X;
                if (viewSource.X > background.X - screenDim.X)
                    viewSource.X = background.X - screenDim.X;
            }
            ...
            if (viewSource.Y < background.Y - screenDim.Y && mouseDiff.Y > 0)
            {
                viewSource.Y += mouseDiff.Y;
                if (viewSource.Y > background.Y - screenDim.Y)
                    viewSource.Y = background.Y - screenDim.Y;
            }
        }
